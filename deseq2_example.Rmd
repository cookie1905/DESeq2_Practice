...
```
# no tissue type is over- or under-represented within any cancerType


# cancerType: Captures the effect of cancerType independent of tissueType.
#             Overall effect of cancerType averaged across all tissueType.


# tissueType: Captures the effect of tissue type independent of cancerType.
#             Overall effect of tissueType averaged across all cancerTypes.
...
# DESeq() runs the following in below order:
...
# design = ~ cancerType + tissueType → results(dds) gives tissueType results.
# design = ~ tissueType + cancerType → results(dds) gives cancerType results.
...
```
# Intercept: expression for cancerType adeno and tissueType Normal.
# cancerType_squamous_vs_adeno: Effect of condition squamous vs adeno, controlling for tissueType.
# tissueType_Stroma_vs_Normal: Effect of tissueType Stroma vs Normal controlling for cancerType.
# tissueType_Tumor_vs_Normal: Effect of tissueType Tumor vs Normal controlling for cancerType.
...
```{r}
topGenes <- rownames(counts(dds)[topExpressed, ])
...
topGenesDF[!is.na(topGenesDF$GeneSymbol), 2]

```

```{r}
rownames(counts(dds)[topExpressed, ])
```
...
ggplot(pcaData, aes(PC1, PC2, color=tissueType, shape=cancerType)) +
  ...
  coord_fixed()
```
# Adding interactions to design
# How the effect of one variable depends on the level of the other.

# cancerType:tissueType: whether the effect of cancerType depends on tissueType.
#                        or
#                        whether the effect of tissueType depends on cancerType.

# Captures when change in expression due to cancerType adeno and squamous is 
# different for tissueType normal, sqamous and tumor 


```{r}
ddsInt <- dds
design(ddsInt) <- ~ cancerType + tissueType + cancerType:tissueType
ddsInt <- DESeq(ddsInt)
```
...
# Interaction coefficients are only for non-reference × non-reference combinations.
# Reference levels are omitted

# If we only want to know how tissueType effects differ between cancerTypes
# Not the main effects, then use

```{r}
ddsInt2 <- dds
ddsInt2$group <- factor(paste0(ddsInt2$tissueType, ddsInt2$cancerType))
design(ddsInt2) <- ~ group
ddsInt2 <- DESeq(ddsInt2)
```

```{r}
resultsNames(ddsInt2)
```
```{r}
levels(ddsInt2$group)
```
